...
def visual_fea(inp,channel=[2,3,4],name='laoda.png'):
    ...
    plt.scatter(inp_tsne[:, 0], inp_tsne[:, 1], c=color_dict[4], s=100, edgecolor='k')
    plt.scatter(inp_tsne[channel[0], 0], inp_tsne[channel[0], 1], c=color_dict[1], marker='s', s=150, label='Variate 1')  # 正方形
    plt.scatter(inp_tsne[channel[1], 0], inp_tsne[channel[1], 1], c=color_dict[1], marker='^', s=200, label='Variate 2')  # 三角形
    plt.scatter(inp_tsne[channel[2], 0], inp_tsne[channel[2], 1], c=color_dict[1], marker='*', s=240, label='Variate 3')  # 五角星形
...
def visual_forecast(inp1, inp2=None,inp3=None,inp4=None,inp5=None,inp6=None,name='./predcition_res.pdf'):
    ...
    if inp1 is not None:
        plt.plot(inp1[0], label=f'{inp1[1]}', linewidth=8,color=color_dict[inp1[2]],linestyle='-')
    if inp2 is not None:
        plt.plot(inp2[0], label=f'{inp2[1]}', linewidth=8,color=color_dict[inp2[2]],linestyle='-')
    if inp3 is not None:
        plt.plot(inp3[0], label=f'{inp3[1]}', linewidth=8,color=color_dict[inp3[2]],linestyle='-')
    if inp4 is not None:
        plt.plot(inp4[0], label=f'{inp4[1]}', linewidth=8,color=color_dict[inp4[2]],linestyle='-')
    if inp5 is not None:
        plt.plot(inp5[0], label=f'{inp5[1]}', linewidth=8,color=color_dict[inp5[2]],linestyle='-')
    if inp6 is not None:
        plt.plot(inp6[0], label=f'{inp6[1]}', linewidth=8,color=color_dict[inp6[2]],linestyle='-')
